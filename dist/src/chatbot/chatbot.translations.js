...
exports.readSourceTranslationFile = readSourceTranslationFile;
exports.writeTranslationFile = writeTranslationFile;
exports.translateJsonObject = translateJsonObject;
exports.translateWebsite = translateWebsite;
exports.getLanguageDisplayNames = getLanguageDisplayNames;
exports.getLanguageDisplayName = getLanguageDisplayName;
exports.updateLanguageRegistry = updateLanguageRegistry;
exports.getAvailableLanguageFiles = getAvailableLanguageFiles;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const genai_1 = require("@google/genai");
function getMessagesDirectory() {
    ...
    try {
        ...
        throw new Error(`Messages directory not found. Tried paths: ${possiblePaths.join(', ')}. Please set MESSAGES_DIRECTORY environment variable or ensure the messages folder exists. Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
function readSourceTranslationFile() {
...
async function writeTranslationFile(languageCode, content) {
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
    const apiSecret = process.env.TRANSLATION_API_SECRET;
    if (frontendUrl && apiSecret) {
        try {
            const response = await fetch(`${frontendUrl}/api/languages/${languageCode}`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                console.log(...oo_oo(`433011268_106_8_106_73_4`, `Translation file for ${languageCode} saved via API`));
                return;
            }
            else {
                console.warn(`Failed to save translation via API (${response.status}), falling back to file system`);
            }
        }
        catch (error) {
            console.warn(`Error saving translation via API, falling back to file system:`, error);
        }
    }
    try {
        const messagesDir = getMessagesDirectory();
        ...
        console.log(...oo_oo(`433011268_133_4_133_76_4`, `Translation file for ${languageCode} saved to file system`));
    }
    catch (error) {
        throw new Error(`Failed to write translation file: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
async function translateJsonObject(obj, targetLanguage, apiKey) {
...
${jsonString}`;
    try {
        ...
        if (response?.candidates && response.candidates[0]?.content?.parts) {
            ...
            translatedText = parts.map((part) => part.text || '').join('');
        }
        ...
        console.error(...oo_tx(`433011268_204_4_204_51_11`, 'Error translating JSON:', error));
        throw new Error(`Failed to translate: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
async function translateWebsite(targetLanguage, apiKey) {
    try {
        if (!targetLanguage || targetLanguage.length < 2) {
            return {
                success: false,
                message: 'Invalid language code. Please provide a valid ISO 639-1 language code (e.g., "fr", "es", "de").',
            };
        }
        const sourceTranslations = readSourceTranslationFile();
        const translatedContent = await translateJsonObject(sourceTranslations, targetLanguage, apiKey);
        ...
        console.error(...oo_tx(`433011268_254_4_254_54_11`, 'Error translating website:', error));
        return {
            success: false,
...
const DEFAULT_FRONTEND_LANGUAGES = [
    { code: 'en', name: 'English', nameEn: 'English' },
    { code: 'zh', name: '中文', nameEn: 'Chinese' },
    { code: 'ms', name: 'Bahasa Melayu', nameEn: 'Malay' },
];
async function updateLanguageRegistry(languageCode, languageName, languageNameEn) {
    try {
        ...
        if (frontendUrl && apiSecret) {
            try {
                const currentRegistryResponse = await fetch(`${frontendUrl}/api/languages/registry`);
                let currentRegistry = [];
                ...
                const updateResponse = await fetch(`${frontendUrl}/api/languages/registry`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        Authorization: `Bearer ${apiSecret}`,
                ...
                if (updateResponse.ok) {
                    console.log(...oo_oo(`433011268_380_10_382_11_4`, `Language ${languageCode} added to runtime registry via API successfully.`));
                    return;
                }
                else {
                    console.warn(`Failed to update registry via API (${updateResponse.status}), falling back to file system`);
                }
            }
            catch (error) {
                console.warn(`Error updating registry via API, falling back to file system:`, error);
            }
        }
        const messagesDir = getMessagesDirectory();
        ...
        if (fs.existsSync(registryPath)) {
            try {
                ...
                if (Array.isArray(parsed)) {
                    registry = parsed.filter((entry) => !!entry &&
                        typeof entry === 'object' &&
                ...
                console.warn('Unable to parse existing language registry, recreating file.', error);
            }
        }
        ...
        console.log(...oo_oo(`433011268_444_4_446_5_4`, `Language ${languageCode} added to runtime registry successfully.`));
    }
    catch (error) {
        console.error(...oo_tx(`433011268_448_4_448_61_11`, 'Error updating language registry:', error));
    }
}
function getAvailableLanguageFiles() {
    try {
        const messagesDir = getMessagesDirectory();
...
;
oo_te;
//# sourceMappingURL=chatbot.translations.js.map