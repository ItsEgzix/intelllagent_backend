...
let ChatbotService = class ChatbotService {
    ...
    sessions = new Map();
    ragContext = '';
    constructor(meetingsService, agentsService) {
...
Based on the context above, answer the user's questions. If the question is not related to the context, you can still answer using your general knowledge, but prioritize the context when relevant.`;
    ...
    async scheduleMeeting(params) {
        try {
            ...
            const workingHoursValidation = this.validateWorkingHours(params.date, params.time, params.timezone);
            ...
            if (!agentId && params.agentName) {
                const foundAgentId = await this.findAgentByNameOrEmail(params.agentName);
                if (!foundAgentId) {
            ...
            const tools = [
                {
                    functionDeclarations: [
                        {
                            name: 'list_agents',
                            description: 'Get a list of available agents that can be scheduled for meetings. Use this when the user asks about available agents or wants to see who they can schedule with.',
                            parameters: {
                                type: 'object',
                                properties: {},
                                required: [],
                        ...
                        },
                        {
                            ...
                            description: 'Schedule a call/meeting with an agent. Use this when the user wants to book a meeting or schedule a call.',
                            parameters: {
                                type: 'object',
                                properties: {
                                    customerName: {
                                    ...
                                    date: {
                                        type: 'string',
                                        description: 'Date in YYYY-MM-DD format (e.g., 2024-12-25). Must be a weekday (Monday-Friday), not a weekend.',
                                    },
                                    time: {
                                        type: 'string',
                                        description: "Time in HH:mm format (e.g., 14:30). Must be between 8:30 and 17:30 (5:30 PM) in the customer's timezone.",
                                    },
                                    timezone: {
                                        type: 'string',
                                        description: 'IANA timezone string (e.g., Asia/Kuala_Lumpur, America/New_York)',
                                    },
                                    agentId: {
                                        type: 'string',
                                        description: 'Optional: ID of the agent to schedule with. Prefer using agentName instead.',
                                    },
                                    agentName: {
                                        type: 'string',
                                        description: 'Required if agentId not provided: Name or email of the agent to schedule with. Use list_agents to see available agents.',
                                    },
                                },
                                required: [
                                    'customerName',
